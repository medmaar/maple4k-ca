...
const googleReviews = [
  ...
  { name: "Liam O'Connor", rating: 5, text: "I mainly watch football and PPV events, and Maple4K handles them well. No major buffering so far. Had to ask support about channel categories and they explained it clearly. Much smoother than my previous provider." },
...
// Trustpilot SVG logo — no background, transparent
const TrustpilotLogo = () => (
  <svg viewBox="0 0 260 60" width="180" height="44" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="60" y="30" fontFamily="Arial,sans-serif" fontWeight="700" fontSize="26" fill="#191919" dominantBaseline="middle">Trustpilot</text>
    <g transform="translate(60,38)">
      {[0,1,2,3,4].map(i => (
        <rect key={i} x={i*22} y={0} width={18} height={18} rx="2" fill="#00b67a"/>
      ))}
      {[0,1,2,3,4].map(i => (
        <text key={i} x={i*22+9} y={9} textAnchor="middle" dominantBaseline="middle" fontSize="13" fill="white">★</text>
      ))}
    </g>
  </svg>
);

// Google Reviews SVG logo — no background
const GoogleReviewsLogo = () => (
  <svg viewBox="0 0 280 60" width="220" height="52" xmlns="http://www.w3.org/2000/svg">
    {/* G icon */}
    <path d="M24 12C17.4 12 12 17.4 12 24s5.4 12 12 12c5.6 0 10.3-3.8 11.6-9H24v-4h16.2c.2 1 .3 2 .3 3 0 8.8-5.9 15-16.5 15C12.5 41 6 34.5 6 24S12.5 7 24 7c5.6 0 10.2 2.1 13.8 5.4l-3.6 3.6C31.8 13.6 28.2 12 24 12z" fill="#4285F4"/>
    <path d="M12 24c0-3.3 1.2-6.3 3.2-8.6L9.4 9.6C6.1 13.2 4 18.4 4 24s2.1 10.8 5.4 14.4l5.8-5.8C13.2 30.3 12 27.3 12 24z" fill="#34A853"/>
    <path d="M36 24h4.2c0 1-.1 2-.3 3H36v-3z" fill="#4285F4"/>
    <path d="M24 41c-4.7 0-8.9-1.8-12-4.6l5.8-5.8c1.6 1.6 3.8 2.6 6.2 2.6 2.8 0 5.3-1 7.1-2.8l5.6 5.6C34.1 38.7 29.4 41 24 41z" fill="#FBBC05"/>
    <path d="M6 24c0-5.6 2.1-10.8 5.4-14.4l5.8 5.8C15.2 17.7 14 20.7 14 24H6z" fill="#EA4335"/>
    {/* "oogle" text */}
    <text x="52" y="28" fontFamily="Arial,sans-serif" fontWeight="700" fontSize="26" fill="#4285F4" dominantBaseline="middle">o</text>
    <text x="70" y="28" fontFamily="Arial,sans-serif" fontWeight="700" fontSize="26" fill="#EA4335" dominantBaseline="middle">o</text>
    <text x="88" y="28" fontFamily="Arial,sans-serif" fontWeight="700" fontSize="26" fill="#FBBC05" dominantBaseline="middle">g</text>
    <text x="106" y="28" fontFamily="Arial,sans-serif" fontWeight="700" fontSize="26" fill="#34A853" dominantBaseline="middle">l</text>
    <text x="116" y="28" fontFamily="Arial,sans-serif" fontWeight="700" fontSize="26" fill="#EA4335" dominantBaseline="middle">e</text>
    {/* Reviews + stars */}
    <text x="52" y="50" fontFamily="Arial,sans-serif" fontWeight="600" fontSize="16" fill="#5F6368" dominantBaseline="middle">Reviews</text>
    <text x="140" y="50" fontFamily="Arial,sans-serif" fontSize="16" fill="#FBBC05" dominantBaseline="middle">★★★★★</text>
  </svg>
);

function useAutoSlide(total: number, interval = 5200) {
  const [current, setCurrent] = useState(0);
  const [animating, setAnimating] = useState(false);
  const [direction, setDirection] = useState<'in'|'idle'>('idle');
  ...
  useEffect(() => {
    timer.current = setInterval(() => {
      ...
      setCurrent(c => {
        ...
        setAnimating(true);
        setDirection('in');
        setTimeout(() => { setAnimating(false); setDirection('idle'); }, 480);
        return next;
      });
    }, interval);
  ...
  return { current, go, animating };
}

const slideStyle = (animating: boolean): React.CSSProperties => ({
  transition: animating ? "none" : "opacity 0.5s ease, transform 0.5s cubic-bezier(0.25,0.46,0.45,0.94)",
    ...
    <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 22 }}>
      {Array.from({ length: total }).map((_, i) => (
        <button key={i} onClick={() => go(i)} style={{
          width: i === current ? 26 : 10, height: 10,
...
const TP_STARS = () => (
  <div style={{ display: "flex", gap: 4, marginBottom: 16 }}>
    {[0,1,2,3,4].map(i => (
      <div key={i} style={{ width: 30, height: 30, background: "#00b67a", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 4 }}>
...
export default function ReviewsSection({ showHeader = true }: { showHeader?: boolean }) {
  ...
  const wa = useAutoSlide(Math.ceil(whatsappReviews.length / 2), 5600);
  const g = useAutoSlide(Math.ceil(googleReviews.length / 3), 6000);
    ...
    <div>
      ...
      <div style={{ marginBottom: 64 }}>
        ...
        <div style={{ maxWidth: 780, margin: "0 auto", overflow: "hidden" }}>
          <div style={slideStyle(tp.animating)}>
            <div style={{ background: "#ffffff", borderRadius: 20, padding: "32px 36px", boxShadow: "0 4px 28px rgba(63,154,174,0.13)", border: "1px solid rgba(63,154,174,0.15)" }}>
              <TP_STARS />
      ...
      <div style={{ marginBottom: 64 }}>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", overflow: "hidden" }}>
          <div style={slideStyle(wa.animating)}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ overflow: "hidden" }}>
          <div style={slideStyle(g.animating)}>
